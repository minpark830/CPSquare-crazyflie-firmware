...
class App:
    ...
    def app_packet_received(self, data):
        ...
        match id:
            case 232:
                # follower 2
                try:
                    response = requests.post("http://localhost:5000/followers/2/position", json=payload)
            ...
            case 233:
                # follower 3
                try:
                    response = requests.post("http://localhost:5000/followers/3/position", json=payload)
                    response.raise_for_status()
                except requests.exceptions.RequestException as e:
                    print("Failed to send position:", e)
    ...
    def connect_to_crazyflie(self):
        ...
        self.currentLeader = 'radio://1/80/2M/E7E7E7E7E8'
        self.leader_label.config(text=f"Current Leader: {self.currentLeader}")
        self.cf.open_link('radio://1/80/2M/E7E7E7E7E8?rate_limit=100')
